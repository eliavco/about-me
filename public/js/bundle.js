...
var displayMap = function displayMap(locations, tourDuration) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiZWxpYXZjbyIsImEiOiJjazBydGQ3azkwNWgxM2JwaGs3dms2cWx2In0.6Z-xCrzOEYMrkA9IGBeXGw';
  ...
  locations.forEach(function (loc, i) {
    ...
    var anchorMarker = 'bottom';
    var anchorPopup = 'bottom';
    var offsetPopup = 30;

    if (i === 0) {
      el.className = 'marker-start';
      anchorPopup = 'top';
      offsetPopup = 0; // number of days in duration doesn't include day of departure 0
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: anchorMarker
    }).setLngLat(loc.coordinates).addTo(map).setPopup(new mapboxgl.Popup({
      offset: offsetPopup,
      closeButton: false,
      closeOnClick: true,
      anchor: anchorPopup
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": <a target=\"_blank\" class=\"link-google-maps\" href=\"http://www.google.com/maps/place/").concat(loc.coordinates[1], ",").concat(loc.coordinates[0], "\">").concat(loc.description, "</a></p>"))); // .addTo(map);
  ...
  var coordinatesLine = locations.map(function (loc) {
    ...
    return regeneratorRuntime.wrap(function _callee3$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          ...
          case 3:
            res = _context3.sent;
            defaultRoutes = [/^\/tour/, '/', '/login', '/signup']; // IF reload not set with true the browser might reload the page from the cache and not GET the current page again from the server
            ...
            if (!(window.location.pathname in defaultRoutes)) {
              _context3.next = 8;
              break;
            }

            return _context3.abrupt("return", location.reload(true));

          case 8:
            return _context3.abrupt("return", location.assign('/'));
          ...
          case 11:
            ...
            (0, _alerts.showAlert)('error', 'Error while logging out! Please try again later.');
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'Password' ? '/api/v1/users/updatePassword' : '/api/v1/users/updateInfo';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
          ...
          case 4:
            ...
            //
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', "".concat(type, " Successfully Changed!"));
              setTimeout(function () {
...
var stripe = Stripe('pk_test_G2AZJSnZeT0OX7gCsFG8eWFG002Z9UEJwO');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("".concat(window.location.origin, "/api/v1/bookings/checkout-session/").concat(tourId));
...
var DOMRegistrationForm = document.querySelector('.signup-form');
var DOMLoginForm = document.querySelector('.login-form');
var DOMUpdateInfoForm = document.querySelector('.form-user-data');
...
var DOMBookTourAction = document.getElementById('book-tour');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            (0, _login.signup)(name.value, email.value, password.value, passwordConfirm.value);
            name.value = '';
            email.value = '';
            password.value = '';
...
function () {
  ...
  return function loginForm(_x2) {
    return _ref2.apply(this, arguments);
  };
}();

var purchseTour = function purchseTour(e) {
  e.preventDefault();
  e.target.textContent = 'Processing...'; // tour-id gets converted to tourId
...
function () {
  ...
  return function updateInfoForm(_x3) {
    return _ref3.apply(this, arguments);
  };
}();
...
function () {
  ...
  regeneratorRuntime.mark(function _callee4(e) {
    var email, oldPassword, newPassword, newPasswordConfirm, savePassword;
      ...
      while (1) {
        switch (_context4.prev = _context4.next) {
          case 0:
            ...
            savePassword = document.querySelector('.btn-save-password');
...
if (DOMMap) {
  var tourDuration = DOMMap.dataset.duration * 1;
  var locations = JSON.parse(DOMMap.dataset.locations);
  locations[0].day = 0;
  (0, _mapbox.displayMap)(locations, tourDuration);
}

if (DOMLogoutBtn) DOMLogoutBtn.addEventListener('click', _login.logout);
